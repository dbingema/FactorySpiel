...
class MachineMenu(cocos.layer.Layer):
    ...
    def __init__(self, conveyorInfo, levelInfo):
        ...
        circle = cocos.sprite.Sprite('whiteCircle.png', position=(self.center_x, self.center_y), opacity=80, scale=0.7)
        self.add(circle)
            
        w2, h2 = cancel.width/2, cancel.height/2
        self.machineOptions = {"cancel": (self.center_x-w2, self.center_y-h2, self.center_x+w2, self.center_y+h2)}
...
##        shadowOffset = 2
        ...
        for index, machine in enumerate(self.levelInfo.machines):
            ...
            xi = self.center_x + menuRadius * math.cos(index * angleIncrease + 0.25*math.pi)
            yi = self.center_y + menuRadius * math.sin(index * angleIncrease + 0.25*math.pi)
            # kleines bild hinzufuegen
            option = cocos.sprite.Sprite(image, (xi, yi), scale = 0.6)
##            shadow = cocos.sprite.Sprite(image, (xi-shadowOffset, yi+shadowOffset),
##                                         scale = 0.7, color = (0,0,0), opacity = 50)
##            self.add(shadow)
            self.add(option)
